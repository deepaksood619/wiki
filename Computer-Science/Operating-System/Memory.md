...
## %MEMis directly related to RES, it's the percentage use of total physicalmemory by the process

## VIRTis the total memory that this process has access to shared memory, mapped pages, swapped out pages, etc

## RESis the total physicalmemory used shared or private that the process has access to

## SHRis the total physical shared memory that the process has access to

## DATAis the total private memory mapped to process physical or not

## CODEalso known as "textresident set" is total physical memory used to load application code
